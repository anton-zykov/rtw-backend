...
import { cleanUpAdverbsTasks } from 'test/hooks/index.js';
  ...
  describe('POST /create', () => {
    describe('when valid tasks are provided', async () => {
      it('then should create tasks and return their ids', async () => {
        ...
        expect(resData.length).toEqual(adverbsTasks.length);
        for (const task of resData) expect(task.id).toBeUuidV4();
        expect(await app.prisma.adverbsTask.findMany({
          where: {
            id: {
        ...
        await cleanUpAdverbsTasks(app, adminCookie, resData.map((task: { id: string }) => task.id));
      });
    });
  ...
  describe('DELETE /delete', () => {
    describe('when valid task ids are provided', async () => {
      it('then should delete tasks', async () => {
        ...
        expect(await app.prisma.adverbsTask.findUnique({
          where: {
            id: taskId,
          },
        })).toBeDefined();
