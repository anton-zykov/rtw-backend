...
import { createUser, createTeacher, createStudent, cleanUpUser, cleanUpStressTasks } from 'test/hooks/index.js';
import { afterAll, beforeAll, describe, expect, it } from 'vitest';

describe('/stress-task/exercise', () => {
    ...
    describe('when student has no cached exercise', () => {
      it('then should retrieve default amount (10) of tasks', async () => {
        ...
        const exerciseRes = await app.inject({
          method: 'GET',
          url: `/api/stress-task/exercise/${student.id}`,
  ...
  describe('POST /check', () => {
    describe('when checking answers', () => {
      it('then should return correct results and update weights accordingly', async () => {
        ...
        const checkRes = await app.inject({
          method: 'POST',
          url: '/api/stress-task/exercise/check',
          headers: { Cookie: studentCookie },
          body: {
            userId: student.id,
            exercise: [
    ...
    describe('when weight reaches minimum (1)', () => {
      it('then should not decrease weight below 1', async () => {
        ...
        await app.prisma.studentStressTask.update({
          where: {
            studentId_taskId: {
              studentId,
              taskId,
        ...
        const updatedTask = await app.prisma.studentStressTask.findUnique({
          where: {
            studentId_taskId: {
              studentId,
              taskId,
