...
import { cleanUpStressTasks } from 'test/hooks/index.js';
  ...
  describe('POST /create', () => {
    describe('when valid tasks are provided', async () => {
      it('then should create tasks and return their ids', async () => {
        ...
        expect(resData.length).toEqual(stressTasks.length);
        for (const task of resData) expect(task.id).toBeUuidV4();
        expect(await app.prisma.stressTask.findMany({
          where: {
            id: {
        ...
        await cleanUpStressTasks(app, adminCookie, resData.map((task: { id: string }) => task.id));
      });
    });
  ...
  describe('DELETE /delete', () => {
    describe('when valid task ids are provided', async () => {
      it('then should delete tasks', async () => {
        ...
        expect(await app.prisma.stressTask.findUnique({
          where: {
            id: taskId,
          },
        })).toBeDefined();
        ...
        const res = await app.inject({
          method: 'DELETE',
          url: '/api/stress-task/pool/delete',
          headers: {
            Cookie: adminCookie,
          },
          body: [taskId],
